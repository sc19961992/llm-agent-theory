...
本文档记录两份内部母稿如何迁移到《宏观规律何时存在？》论文包。它是编辑追踪文件，不是论文参考文献，也不主张母稿中的每一句话都进入正式稿。正式论文的定义、定理编号和证明以本目录中的 LaTeX 文件为准。
...
- Transformer 结果采用分布加权 TV；它不被写成正文最坏情形 $\Gamma_N$ 判据的字面特例。完整证明与对齐条件统一置于 `appendices/appendix_J_transformer_predictive_states.tex`，并可通过独立包装文件单独编译。
...
| 母稿单元 | 新位置 | 处理 |
|---|---|---|
| 标题“涌现、构建与闭合：一个统一证明框架” | `main.tex`、`README.md` | 改名为“宏观规律何时存在？有限随机系统中的压缩—闭合判据”，把范围收窄到有限随机系统的任务相对存在判据；原题不保留。 |
...
| 贯穿全文的非平凡正例 | `sections/05_fault_repair_example.tex`；`appendices/appendix_E_examples_counterexamples.tex` | 正文保留故障数链及最小性结论；辅助计算与对照例移入附录。 |
...
| Transformer 构造例与一般框架的关系 | `sections/06_construction_routes.tex`；`appendices/appendix_J_transformer_predictive_states.tex` | 正文只说明结论和非字面特例关系；三个假设、加权定理与完整证明进入附录 J。 |
...
| 时间尺度的位置 | `sections/07_verification_and_limits.tex`、`sections/08_discussion.tex`；`appendices/appendix_G_composition_time.tex` | 一步主定理与多步外推分开；固定时长、增长时长和收缩条件不混写。 |
...
| 1.2 可交换故障—修复链 | `sections/05_fault_repair_example.tex`；`appendices/appendix_E_examples_counterexamples.tex` | 作为纵贯正文的非平凡正例；参数退化与最小性细节放附录。 |
...
| 2.4 任务相对的最小闭合商 | `sections/03_closure_and_fibers.tex`；`appendices/appendix_C_exact_theory.tex` | 正文给出规范性结论；精确构造、普适性质和完整证明入附录。 |
| 2.5 多层复合与有限终止 | `appendices/appendix_G_composition_time.tex` | 全部移出正文；与近似复合统一记号。 |
...
| 3.3 预测状态与 Koopman 表示的边界 | `sections/06_construction_routes.tex`、`sections/08_discussion.tex`；`appendices/appendix_J_transformer_predictive_states.tex` | 预测状态形成 Transformer 附录 J；Koopman 只用于讨论概念边界，不宣称等价。 |
| 4. 范畴化版本 | `appendices/appendix_I_categories.tex` | 完整移出正文；作为组织语言，不增加有限主定理的数学强度。 |
| 5. 近似闭合、系统族与扩展 | `sections/03_closure_and_fibers.tex`、`sections/04_main_criterion.tex`；附录 D、G、H | 有限近似理论留主线，空间复合、时间传播和其他扩展接口移入附录。 |
...
| 5.3.5 两层复合与误差证书范畴 | `appendices/appendix_G_composition_time.tex`；`appendices/appendix_I_categories.tex` | 数值误差复合与范畴解释分开，删除双份证明。 |
| 5.3.6 多层空间复合 | `appendices/appendix_G_composition_time.tex` | 移出正文，保留有限层误差和。 |
| 5.3.7 同一因子的多步时间误差 | `appendices/appendix_G_composition_time.tex`；`sections/07_verification_and_limits.tex` | 附录给线性与收缩界，正文只强调适用条件。 |
| 5.3.8 确定性粗粒化的路径误差 | `appendices/appendix_G_composition_time.tex` | 与状态边缘误差明确区分；不把状态收缩误用到已输出轨迹。 |
| 5.3.9 任务相对复杂度—误差曲线 | `sections/03_closure_and_fibers.tex`、`sections/04_main_criterion.tex`；`appendices/appendix_D_approximate_theory.tex` | 正文先给有限规模前沿，再用其定义 $\Gamma_N$；完整优化性质入附录。 |
| 5.4 系统族的低复杂度近似闭合 | `sections/04_main_criterion.tex`；`appendices/appendix_D_approximate_theory.tex` | 作为充要条件主定理；量词与优化达到完整写明。 |
| 5.4.1 Transformer 分布加权构造例 | `sections/06_construction_routes.tex`；`appendices/appendix_J_transformer_predictive_states.tex` | 从一般证明中拆出，自包含重证；新增与 $\Gamma_N$ 对齐的三项条件。 |
...
| 母稿单元 | 新位置 | 处理 |
|---|---|---|
| 标题“从底层动力学到上层规律……” | `main.tex`、`README.md` | 不沿用；改为问题式标题“宏观规律何时存在？”，并以副标题明确有限系统与压缩—闭合判据。 |
...
| 4.3 预测状态 | `sections/06_construction_routes.tex`；`appendices/appendix_J_transformer_predictive_states.tex` | 正文说明两层结论与关系，完整构造和证明见附录 J。 |
| 5. 预测状态为何闭合 | `appendices/appendix_J_transformer_predictive_states.tex` | 重写为“递推 + 同一 token 推前”的严格证明。 |
| 6. 三个条件 | `appendices/appendix_J_transformer_predictive_states.tex` | 统一为 T1 语言结构、T2 总体学习、T3 状态可读出。 |
| 6.1 统一记号与系统边界 | `appendices/appendix_J_transformer_predictive_states.tex` 的“系统边界与统一记号” | 保留有限词表、有限时域、EOS、分布与 Markov 完整状态；删去重复说明。 |
...
| 7. Transformer 自回归预测状态定理 | `appendices/appendix_J_transformer_predictive_states.tex` | 改名为分布加权闭合定理，避免单独使用“涌现”造成过度解释。 |
...
| 8. 从一步预测到整段生成 | `appendices/appendix_J_transformer_predictive_states.tex`；`appendices/appendix_G_composition_time.tex` | Transformer 特定固定时长边缘界入附录 J；一般多步与路径界入附录 G。 |
| 8.1 上层模拟器如何运行 | `sections/06_construction_routes.tex`；附录 J 的 $R_N,F_N,L_N$ 定义 | 不保留操作步骤列表；由 kernel 定义直接表达。 |
| 8.2 宏观状态边缘多步误差 | 附录 J 命题 `transformer-fixed-horizon`；`appendices/appendix_G_composition_time.tex` | 补足 rollout 分布前提；状态边缘与 token 轨迹明确分开。 |
| 9. 最简可行规律存在 | `sections/03_closure_and_fibers.tex`、`sections/04_main_criterion.tex`；`appendices/appendix_D_approximate_theory.tex` | 由有限分区优化达到与 $\Gamma_N$ 判据统一处理，不在附录 J 重证。 |
| 10. 对应真实 Transformer | `sections/07_verification_and_limits.tex`；`appendices/appendix_J_transformer_predictive_states.tex` | 分成形式边界、部署核验和定理边界，不作为无条件现实结论。 |
| 10.1 固定推理过程有限 Markov 化 | 附录 J“系统边界与统一记号”；`sections/07_verification_and_limits.tex` | 保留有限精度、有限历史与完整状态条件；连续/无界环境列为范围外。 |
| 10.2 最终输出任务通道 | `sections/07_verification_and_limits.tex`；`appendices/appendix_G_composition_time.tex` | 归入一般任务通道与后处理，不与下一 token 定理混为同一任务。 |
| 10.3 代理误差相加 | `sections/07_verification_and_limits.tex`；`appendices/appendix_D_approximate_theory.tex` | 归入核估计/部署失配的三角不等式；避免附录 J 主线膨胀。 |
| 11. 结论层级与边界 | `sections/07_verification_and_limits.tex`；附录 J“结论边界” | 去重后分别保留一般理论与 Transformer 专项边界。 |
| 11.1 一般有限系统 | `sections/07_verification_and_limits.tex` | 以正式稿定理编号重写，不复制旧表。 |
| 11.2 Transformer 自回归情形 | 附录 J“结论边界”；`sections/07_verification_and_limits.tex` | 区分已证的条件结论和仍需验证的 T1--T3。 |
| 11.3 行为规律、内部表征与低维几何 | 附录 J 的 T3 与“结论边界”；`sections/08_discussion.tex` | 保留三层区分：行为商不推出指定层读出，更不推出低维流形。 |
| 结语 | `sections/08_discussion.tex`；附录 J 开头与结论边界 | 不照录传播性口号；由条件、结论和未证内容共同收束。 |

## 正式稿中明确不作的过度声称

1. 不使用“有限复杂系统必然有简单宏观规律”的无条件句式；正文只证明 $\Gamma_N\to0$ 与指定低复杂度闭合性质等价，并给出满足条件的系统族。
