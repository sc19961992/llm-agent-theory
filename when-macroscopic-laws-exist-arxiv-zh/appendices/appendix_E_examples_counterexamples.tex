...
本附录给出五类判据边界：循环链的微观尺度障碍、无非平凡状态置换对称的精确闭合、
...
\label{counter:cycle}
对 $N\ge2$，令 $B_N=\mathbb Z/N\mathbb Z$，确定性核为
$K_N(x,x+1)=1$，任务为 $g_N(x)=\one_{\{x=0\}}$。若 $0\le\tau<1/2$，
任何任务保真且闭合缺陷不超过 $\tau$ 的分区都只能是离散分区，故
\[
c_{K_N}^{\tau}(g_N)=N,
\]
该系统族的相对状态复杂度保持为 $1$，不满足 LC。
\end{counterexample}
...
\subsection{无非平凡状态置换对称的精确闭合}

\begin{example}[无非平凡状态置换对称的闭合商]
...
\end{pmatrix}.
\]
前两行到两个分区块的概率都为 $(0.7,0.3)$，所以 $\pi$ 精确闭合。三个
对角元 $K(1,1)=0.1$、$K(2,2)=0.5$、$K(3,3)=0.7$ 两两不同；任何满足
$K(hi,hj)=K(i,j)$ 的状态置换 $h$ 都必须逐点固定，故该核没有非平凡状态
置换对称。群等变性提供一种充分构造，精确闭合也可以由其他结构产生。
\end{example}
...
$G_N=H_N=\mathfrak S_N$ 按坐标置换作用。把所有对象放在单点概率空间上。
...
$K_N=\Id_{B_N}$，$\mu_N=\delta_{(+,1)}$，并仍取单点概率空间。此时
...
\]
因此群作用分离事件可取
$\beta_N=\gamma_N=\kappa_N=0$、$\ell_N=1$ 而恒成立。恒等任务迫使
每个任务保真表示都是单射，尤其
\[
c_{\{K_N\}}^\tau(g_N)=|B_N|
