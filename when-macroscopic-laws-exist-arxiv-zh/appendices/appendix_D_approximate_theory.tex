\section{近似闭合：最优核、纤维直径与复杂度--闭合误差前沿}
\label{app:approximate-theory}

本附录以交换式缺陷定义近似闭合，并给出主判据的闭合项。纤维半径--直径
关系把含未知宏观核的最优闭合误差化为可由分区直接计算的预测直径。有限维
紧性保证下述最优值达到；相应计算效率需要另行分析。
...
$\eps$-近似动力学因子。这里 $K,L$ 保持严格归一化，$\eps$ 仅度量交换关系
的缺陷。
...
$L\mapsto QL$ 可能不单射。下节的确定性满射情形由满射性保证零误差宏观核
唯一。
...
定理~\ref{thm:app-four-equivalences}，这正是强可聚合性。共同的
纤维分布逐行唯一确定宏观核。
\end{proof}

界~\eqref{eq:app-diameter-radius} 给出介于 $D_q/2$ 与 $D_q$ 之间的包围半径。
后文使用其零点和渐近趋零等价；精确 Chebyshev 半径可以严格大于 $D_q/2$。
...
\end{equation}
这里同一个 $q$ 适用于整族动力学，宏观核 $L_K$ 可以随 $K$ 改变，因而量词
顺序为\emph{$\sup_K\min_L$}。共享单一宏观核对应
\begin{equation}
...
\end{equation}
正文的系统族判据采用前一种量词；对所有 $K$ 通用的转移核需要另行验证两种
极小极大量相等或直接控制式~\eqref{eq:app-min-sup}。
...
\end{equation}
因此 $D_q(\cK)\to0$ 与本文所用
$\sup_K\min_L$ 缺陷趋零等价到固定常数，并可直接由分区预测直径检验。

\subsection{核族的任务相对复杂度--闭合误差前沿}
...
\begin{theorem}[核族复杂度--闭合误差前沿的有限达到]
...
对每个固定分区与每个固定 $K\in\cK$，内层最优宏观核也达到。
并且：
\begin{enumerate}
  ...
  \end{equation}
\end{enumerate}
这里的外层量是上确界，并允许其不由某个 $K$ 达到。
\end{theorem}

\begin{proof}
有限集只有有限多个分区，$\mathfrak P_m(g)$ 非空且有限；例如离散分区在
$m=|X|$ 时属于其中。对每个固定分区与固定 $K$，
定理~\ref{thm:app-optimal-kernel} 给出最优宏观核。核族允许为无限集，
$\eta_{q_\pi}(\cK)$ 对每个分区是一个良定义的上确界；外层只在有限个
...
达到同一最优状态数或同一前沿值的分区一般不可比较。近似理论的规范对象是
前沿及各点的全部最优解集合。
...
最后给出正文 $\Gamma$ 判据所需的直接稳健形式。设同一有限可辨微观空间
