...
\emph{所有}微观初态分布给出同一宏观下一步分布。本文因此采用强可聚合性
（strong lumpability）；弱版本则把初始分布限制在指定类别中。
...
任务观测在这两个始终存在的端点之间指定不可丢弃的信息，由此形成非退化的
压缩问题。

\subsection{核族的任务相对最小精确商}

正文要求同一个表示服务于一族允许动力学，而允许每个微观核有自己的
宏观核。设 $g:X\twoheadrightarrow O$ 是非恒定任务观测，
...
\end{equation}
若按本文约定 $g$ 非恒定，则左端至少为 $2$。一般有限 Markov 核族可以使
右侧取等号，此时最粗精确任务商为离散分区。
...
$q_*$。这给出有限输入上的构造算法；其在大状态空间中的计算复杂度需要另行
分析。
