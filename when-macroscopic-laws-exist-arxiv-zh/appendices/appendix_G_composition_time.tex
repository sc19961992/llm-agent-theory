...
本附录区分两种不同的累积：多个表示层依次粗粒化是\emph{空间复合}，同一
近似因子反复演化是\emph{时间迭代}。二者使用同一全变差工具，但量词和可
解释的结论不同。这里的时间结论始终从一个已经给定的近似闭合证书出发；它们
不把正文的一步 $\Gamma_N$ 判据自动升级为任意增长时间窗上的结论。
...
\]
是 Markov 通道，其中记号只表示其源、靶动力学，并不预设交换式精确成立。
...
\end{equation}
这里 $\rho_Q$ 控制的是表示通道 $Q$ 的 Dobrushin 系数，不是宏观动力学
$L$ 的混合率。若 $Q$ 是非恒定确定性通道，通常只能取 $\rho_Q=1$。
...
\]
只记录误差而丢弃 $\rho$ 时仍有加法上界，但固定某个正误差阈值的未标记
通道集合一般不对复合封闭。
...
式~\eqref{eq:app-multistep-contracting} 只控制时间 $n$ 的\emph{终端宏观
分布}。它不说明整段路径的早期偏差被后来混合所删除。
...
\le\eps+\beta+\rho\|\nu-\pi_L\|_{\TV}
\]
移项即得。这里的长期稳定性来自近似平稳性与宏观收缩，不来自相结构或
“对称破缺”这一名称本身。
\end{proof}
...
确定性假设不能直接删去。若 $Q$ 是随机通道，方程 $KQ=QL$ 只约束每一时刻
的通道输出边缘；它没有规定跨时间如何联合采样输出。例如取
$K=\Id_X$、$L=\Id_Y$，并令 $Q$ 的某行是非退化分布，则交换式精确成立，
但若每个时刻独立地从 $Q(X_t,\cdot)$ 重采样，所得输出路径会变化，而
$L$-链保持不变。随机粗粒化的路径结论需要额外的联合耦合或 link consistency
条件。

\subsection{增长时间尺度上需要什么}
...
\begin{enumerate}
 ...
 $\eps_N/(1-\rho)\to0$ 控制所有 $T_N$ 的终端宏观边缘；但整段路径仍只由
 式~\eqref{eq:app-path-bound} 一般性控制，收缩不能抹去路径中已经记录的偏差。
\end{enumerate}

一步误差趋零本身不足以控制任意增长时长。一个最小示例是
$X=Y=\{0,1\}$、$Q=\Id$、$L=\Id$，而 $K_\eps$ 从 $0$ 以概率
$\eps$ 跳到吸收态 $1$、从 $1$ 保持不动。此时
\[
 d_\infty(K_\eps Q,QL)=\eps,
\]
但从 $0$ 出发，在 $n$ 步后与 $L$-链终端分布的全变差距离为
\[
 1-(1-\eps)^n.
\]
取 $n=\lfloor1/\eps\rfloor$，该量不趋于零。这个例子只说明\emph{给定近似
因子证书}的传播边界；由于这里 $Q=\Id$，若重新把 $L$ 与表示一同优化当然可
取 $L=K_\eps$ 并得到零缺陷。

因此，正文的系统族主定理是一步陈述。把它提升到固定有限时间是直接推论。
在增长时间窗上，终端边缘需要式~\eqref{eq:app-growing-horizon-alpha}、统一
混合或其他明确的稳定性条件；完整路径一般需要
式~\eqref{eq:app-growing-horizon-linear}或更强的路径级耦合条件。宏观收缩
只能改善终端边缘界，不能抹去路径中已经发生并被记录的偏差。这两类强化都
不能只从 $\Gamma_N\to0$ 推出。
