...
下述系数源于 Dobrushin 对非齐次 Markov 链的工作；其作为全变差收缩系数的现代算子表述可参见\citet{Dobrushin1956,GaubertQu2015}。相关文献同时使用最大行距离及其互补的行重叠系数，形成两种符号约定；本文以 $\alpha$ 明确定义下式的收缩因子。下面给出有限情形的自足证明，以固定行随机约定。
...
非恒定情形满足 $\alpha(Q_q)=1$。因此，确定性粗粒化在最坏状态度量下的
全变差 Lipschitz 常数通常等于 $1$。
\end{remark}

后文所有近似结论都使用同一个 $d_\infty$。更换为平均分布加权距离会改变
量词和可见的坏状态；二者之间的换算需要另行给定采样分布及相应覆盖条件。
