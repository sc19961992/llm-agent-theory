...
\]
只记录误差而丢弃 $\rho$ 时仍有加法上界，但固定某个正误差阈值的态射集合
一般不对复合封闭。
...
\begin{enumerate}
 \item 对每个固定 $T$，$\eps_N\to0$ 足以使终端误差和确定性粗粒化的路径
  误差趋零。
 \item 若 $T_N\to\infty$ 且没有统一收缩信息，一个直接充分条件是
 \begin{equation}
...
把它提升到随规模增长的时间尺度，必须另加
式~\eqref{eq:app-growing-horizon-alpha}、统一混合、或其他明确的稳定性条件，
不能只从 $\Gamma_N\to0$ 推出。
