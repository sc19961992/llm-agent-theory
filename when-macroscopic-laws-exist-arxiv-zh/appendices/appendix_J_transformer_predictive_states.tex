...
\section{自回归模型的递推下一-token表示：分布加权闭合与
\texorpdfstring{$\Gamma_N$}{Gamma-N} 判据}
\label{app:transformer-predictive-states}

本节给出一个自包含的有限状态定理。它适用于边界固定的自回归生成器，只使用实际采样条件分布、完整 Markov 状态和 token 驱动的状态更新；Transformer\citep{VaswaniEtAl2017} 是主要实例，证明本身不依赖 attention 或特定参数化。假设参考条件分布具有相对状态数受控的递推下一-token表示，模型条件分布满足总体 KL 界，且该表示与模型完整状态相容。定理据此控制给定表示的分布加权下一-token误差与一步闭合误差。
...
\paragraph{假设 T1（低相对状态数的递推下一-token表示）.}
...
\]
式 \eqref{eq:T1-recursion} 提供闭合所需的递推结构；T1b 单独给出的下一-token读出误差只控制输出分布。
...
\begin{theorem}[递推下一-token表示的分布加权闭合]
...
\begin{enumerate}
 ...
 $\bar g_N=\widetilde g_N\circ\bar q_N$。正文 $\bar g_N$ 是确定性任务映射，下一-token预测是随机通道。可以另选一个被 $\bar q_N$ 精确保留的确定性任务，也可以在扩展指标中显式加入随机任务通道的缺陷；两种方案分别对应精确与近似任务约束。

 \item \textbf{系统族量词不同。}
 加权定理固定模型、解码规则、参考分布及 T1--T3 中的表示；$\Gamma_N$ 对预先声明的 kernel 族取最坏情形，并在全部任务保真分区中优化。所需条件是同一个分区 $\pi_N$ 对族中每个 kernel 都满足统一闭合界，且
 $|\pi_N|/|B_N|\to0$。整个系统族共享同一个宏观 kernel 对应“存在一个共同的 $L_N$”这一更强量词。
\end{enumerate}
...
$e_N/\omega_N$。此时命题中可取 $r_N=e_N/\omega_N$；较小的 $\omega_N$ 相应放大该上界。

\subsection{定理的适用范围}

在假定递推表示及其完整状态读出存在后，定理证明模型对该表示的分布加权下一-token误差与一步闭合误差受控。现实应用分别需要验证 T1 的参考过程结构、T2 的总体泛化界、指定神经层版本的 T3、计算可发现性以及增长时长条件。Hidden states 的低维流形结构是独立的几何命题。
