\section{单个有限系统：从精确商到复杂度--闭合误差前沿}
\label{sec:closure}
...
$q=s\circ p$；$Q_p,Q_s,Q_q$ 表示相应的确定性核。
...
\begin{proof}
由 $Q_q=Q_pQ_s$ 与 $KQ_p=Q_p\bar K$，等式两边所比较的两对核
...
强可聚合性（strong lumpability）条件\citep{KemenySnell1976,Buchholz1994}：同一
$s$-纤维中的状态必须对每个下一宏观块给出相同转移概率。

单个有限链满足上述条件的最粗分区可由稳定分区细化构造；Derisavi 等称其为
ordinary lumping
\citep{DerisaviEtAl2003}。若把允许核族中的每个核视为 MDP 的一个动作，并把
任务值视为必须保留的状态标签或奖励标签，则下述共同精确闭合条件正是
reward-respecting stochastic bisimulation 的齐次稳定分区条件；最粗稳定细化
及其最小商模型属于经典 MDP 最小化理论\citep{GivanDeanGreig2003}。因此，
以下定理主要把这一精确端点写成适合本文任务映射与核族记号的形式，并为后续
近似前沿提供自包含的零误差基准；同一个表示服务于整个允许动力学族，每个
微观核可以诱导自己的宏观核。
...
\begin{proof}
$B$ 只有有限个任务保真分区，固定分区的宏观核空间是紧单纯形，故
