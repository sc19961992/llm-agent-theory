...
第~\ref{sec:operational}节固定了操作性微观空间 $B$，从而排除了仅由编码
复制产生的虚假压缩。本节先证明在 $B$ 上研究闭合不会改变原系统中的闭合
误差，再依次回答两个单系统问题：零误差时最少需要多少宏观状态；允许误差
...
这说明操作性约化并非只改变复杂度分母：对所有按定义经 $B$ 因子化的候选
...
以下有限精确结论给出近似理论的零误差基准。这里同一个表示必须服务于整个
允许动力学族，但每个微观 kernel 可以诱导自己的宏观 kernel。
...
确定了复杂度--误差图的零误差端点；它不保证式~\eqref{eq:strict-exact-compression}
的右侧严格成立。
...
\label{eq:eta-pi}
\end{equation}
同一个 $\pi$ 服务于全部 $\bar K$，但 $L$ 可以依赖 $\bar K$；因此本文
使用 $\sup_{\bar K}\min_L$。要求整个动力学族共享同一个 $L$ 的
$\min_L\sup_{\bar K}$ 是另一项更强问题。命题~\ref{prop:operational-defect-invariance}
...
$\mathcal E_{\bar\cK,\bar g}$ 及每一点的最优解集合，而不是预先指定的某个
“最佳分区”。
...
\end{equation}
它不含未知的宏观 kernel，只检查将被合并的状态是否对下一宏观块给出一致
预测。
