...
上一节为每个有限系统给出了一条达到的复杂度--误差前沿。现在考虑系统规模
增长时，这条前沿能否同时逼近低相对复杂度与低闭合误差。设
\[
(X_N,\cK_N,u_N,g_N),
\qquad |B_N|\longrightarrow\infty,
\]
并沿用第~\ref{sec:operational}节的操作性微观空间 $B_N$、诱导任务
...
\label{eq:LC}
\end{equation}
这要求同一列任务保真表示同时承担两件事：其状态数相对于操作性微观基线
趋零，其最优一步闭合误差也趋零。
...
$\ell^\infty$ 距离；第二行是不需要先计算整条前沿的直接形式。两式相等：
...
固定候选和每个固定 $\bar K$，最优宏观 kernel 也实际达到。近似最优分区
本身不必唯一。
...
条件 2 中的最优宏观 kernel 允许随 $N$ 和 $\bar K$ 改变；要求整个允许
动力学族共享同一个 $L$ 是更强的 $\min_L\sup_{\bar K}$ 问题。定理也没有
断言所有有限系统族都满足条件：它把存在性化为同一列任务保真分区上的联合
...
有限达到只说明满足给定目标的最优对象存在，不说明它能被高效发现，也不
说明其描述长度、参数量或计算成本最小。$\Gamma_N$ 的实际检验范围和估计
误差见第~\ref{sec:verification}节；循环链反例说明有限 Markov 性本身不能
推出 $\Gamma_N\to0$，见附录~\ref{app:examples-counterexamples}。
