...
\subsection{任务相对最小精确商}

设 $g:X\twoheadrightarrow O$ 是非恒定任务观测。令
\begin{equation}
  \cA_0:=g^*\mathbb R^O,
  \qquad
  \cA_{n+1}:=\operatorname{alg}(\cA_n\cup P_K\cA_n).
  \label{eq:app-task-algebra-iteration}
\end{equation}

\begin{theorem}[任务相对的最小精确宏观模型]
\label{thm:app-minimal-task-quotient}
迭代~\eqref{eq:app-task-algebra-iteration} 在某个
$\tau\le |X|-|O|$ 处稳定；记稳定代数为 $\cA_*$，商为
$q_*:X\twoheadrightarrow X_*$。则：
\begin{enumerate}
  \item $\cA_*$ 是包含 $\cA_0$ 的最小 $P_K$-不变观测代数；
  \item 存在唯一 $K_*\in\Stoch(X_*,X_*)$ 与唯一 $r_*:X_*\to O$，使
  \begin{equation}
    KQ_{q_*}=Q_{q_*}K_*,
    \qquad
    g=r_*\circ q_*;
  ...
  \end{equation}
  \item 若另有满射 $q:X\twoheadrightarrow Y$、读出 $r:Y\to O$ 与
  $L\in\Stoch(Y,Y)$ 满足
  \[
    g=r\circ q,
    \qquad
...
$|X|-|O|$ 次严格增加。稳定时
$P_K\cA_*\subseteq\cA_*$。若 $\mathcal B$ 是任意包含 $\cA_0$ 的
$P_K$-不变观测代数，则从 $\cA_0\subseteq\mathcal B$ 出发归纳得到
$\cA_n\subseteq\mathcal B$，故 $\cA_*\subseteq\mathcal B$。这证明第一项。

定理~\ref{thm:app-closed-algebra} 给出唯一 $K_*$。又因
$\cA_0\subseteq\cA_*$，$g$ 在 $q_*$ 的纤维上常值，故定义
$r_*(q_*(x)):=g(x)$；它良定义且因 $q_*$ 满射而唯一。

对第三项，候选 $q$ 的分区代数
$\cA_q:=q^*\mathbb R^Y$ 包含 $\cA_0$，且由精确交换式和
定理~\ref{thm:app-four-equivalences} 对 $P_K$ 不变。最小性给出
$\cA_*\subseteq\cA_q$，所以 $q(x)=q(x')$ 推出 $q_*(x)=q_*(x')$；
存在唯一满射 $s$ 使 $q_*=s\circ q$。由
$r\circ q=g=r_*\circ s\circ q$ 与 $q$ 满射，得到 $r=r_*\circ s$。
最后，
\[
 Q_qLQ_s=KQ_qQ_s=KQ_{q_*}
 =Q_{q_*}K_*=Q_qQ_sK_*.
\]
满射 $q$ 的各行覆盖 $Y$ 的全部 Dirac 分布，因此可消去左侧的 $Q_q$，
得到 $LQ_s=Q_sK_*$。
\end{proof}

\begin{definition}[精确任务相对自治复杂度]
...
\begin{equation}
 ...
 \exists L\in\Stoch(Y,Y),\quad
 g=r\circ q,\ KQ_q=Q_qL
 \end{array}\right\}.
 \label{eq:app-exact-complexity-min}
\end{equation}
因此，任务相对的严格精确压缩存在，当且仅当
\begin{equation}
  |O|\le c_K(g)<|X|.
  \label{eq:app-nontrivial-exact}
\end{equation}
若按本文约定 $g$ 非恒定，则左端至少为 $2$。该结论不保证右侧严格不等式：
一般有限 Markov 核可能迫使任务分区一直细化到离散分区。
...
$q_*$。这给出有限模型上的构造算法，但没有给出大状态空间中的计算效率保证。
